...
def get_subtitles(video_id, format_type='txt', language='fr'):
    """
    Récupère les sous-titres avec yt-dlp - compatible 2026
    """
    try:
        ...
        ydl_opts = {
            ...
            'socket_timeout': 30,
            'extract_flat': False,
            # FIX 2026 : forcer les clients qui ne nécessitent pas de PO Token
            'extractor_args': {
                'youtube': {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            transcript_data = []
            if json3_format.get('ext') == 'json3' or raw.strip().startswith('{'):
                json_data = json.loads(raw)
                transcript_data = parse_youtube_json(json_data)
            else:
                # Fallback XML (format srv1/ttml)
                transcript_data = parse_xml_subtitles_simple(raw)
...
def parse_xml_subtitles_simple(xml_content):
    """Parse basique du format XML/ttml YouTube en fallback"""
...
def get_available_languages(video_id):
    ...
    try:
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            # FIX 2026 : même fix que get_subtitles
            'extractor_args': {
                'youtube': {
                    'player_client': ['tv_downgraded', 'android'],
                }
